"""Notion MCP server with NBJM (Notion Is Basically Just Markdown) format.

Provides token-efficient Notion access via two tools:
- notion.read: Read pages/databases in compact NBJM format
...
try:
    ...
    _SERVER_VERSION = f"v{_pkg_version('nbjm-mcp')}"
except Exception:
...
def _append_nbjm_line(
    ...
    """Append a NBJM-formatted line with mode-aware ID prefix.
...
class ParseState(Enum):
    """State machine states for NBJM parsing."""
    HEADER = auto()
    BLOCK = auto()
    CODE = auto()
...
class NbjmBlock:
    ...
    children: list["NbjmBlock"] = field(default_factory=list)
...
class NbjmParseError(Exception):
    """Error during NBJM parsing."""
...
@dataclass
class NbjmParseResult:
    """Result of parsing a NBJM document."""
    header: NbjmHeader
    blocks: list[NbjmBlock]
    errors: list[NbjmParseError] = field(default_factory=list)
...
# =============================================================================
# Notion Rich Text → NBJM Conversion
# =============================================================================

def notion_rich_text_to_nbjm(rich_text: list[dict]) -> str:
    """Convert Notion rich_text array to NBJM inline formatting.

    Args:
        rich_text: Notion API rich_text array.

    Returns:
        NBJM formatted string.
    """
    if not rich_text:
        return ""
    ...
    for item in rich_text:
            ...
            if result:
                ...
                if annotations.get("code"):
                    ...
                    if color and color != "default":
                        color_nbjm = color.replace("_", "-")
                        result = f":{color_nbjm}[{result}]"
        ...
        if annotations.get("code"):
            ...
            color = annotations.get("color", "default")
            if color and color != "default":
                # Convert underscore to dash for NBJM format
                color_nbjm = color.replace("_", "-")
                result = f":{color_nbjm}[{result}]"
...
def _text_to_rich_text(
    ...
    """Convert NBJM-formatted text to Notion rich_text array.
    ...
    Args:
        text: NBJM-formatted text (may contain @mentions, **bold**, etc.)
        registry: Optional ID registry for resolving page mention short IDs.
...
def parse_nbjm(nbjm_text: str) -> NbjmParseResult:
    """Parse a NBJM document into structured form.

    Args:
        nbjm_text: The NBJM text to parse.

    Returns:
        NbjmParseResult with header, blocks, and any errors.
    """
    lines = nbjm_text.split('\n')
    header = NbjmHeader()
    blocks: list[NbjmBlock] = []
    errors: list[NbjmParseError] = []

    state = ParseState.HEADER
    current_code_block: Optional[NbjmBlock] = None
                    ...
                    if keyword == 'nbjm':
                        try:
                            ...
                            errors.append(NbjmParseError(
                                ...
                                message=f"Invalid NBJM version: {value}",
                                line=line_num,
                                excerpt=f"{line_num}|{line}"
                            ))
                    elif keyword == 'page':
    ...
    return NbjmParseResult(header=header, blocks=blocks, errors=errors)
...
# Structural block types: NBJM name → Notion block type
...
# Property type to NBJM type mapping
PROPERTY_TYPE_MAP = {
    "title": "title",
    "rich_text": "text",
    "number": "number",
...
# Reverse mapping: NBJM short name → Notion API type
# Used by +db and +prop to convert property definitions to API format
NBJM_TO_API_TYPE = {v: k for k, v in PROPERTY_TYPE_MAP.items()}
...
class PropertyDef:
    ...
    name: str
    nbjm_type: str          # NBJM short name (e.g., "select", "text")
...
def parse_property_def(text: str) -> tuple[Optional[PropertyDef], Optional[str]]:
    ...
    if colon_idx == -1:
        # No config, just type
        nbjm_type = inner.strip()
        config_str = ""
    else:
        nbjm_type = inner[:colon_idx].strip()
        config_str = inner[colon_idx + 1:].strip()

    # Validate NBJM type
    if nbjm_type not in CREATABLE_PROPERTY_TYPES:
        return None, (
            f"Unsupported property type '{nbjm_type}' in: {text}. "
            f"Supported types: {', '.join(sorted(CREATABLE_PROPERTY_TYPES))}"
        )

    api_type = NBJM_TO_API_TYPE[nbjm_type]
    ...
    if nbjm_type in ("select", "multi_select"):
        ...
        return None, f"Type '{nbjm_type}' does not accept config in: {text}"
...
# Operator → Notion condition name, keyed by NBJM property type
...
# Mapping from Notion block type to NBJM marker
...
def render_column_to_nbjm(
    ...
    Returns:
        List of NBJM lines for this column and its children.
    """
    lines = []
    ...
    _append_nbjm_line(lines, mode, short_id, indent, f"!col {column_number}")

    # Render column's children
    children = block.get("_children", [])
    for child in children:
        lines.extend(render_block_to_nbjm(child, registry, level + 1, mode))

    return lines


def render_block_to_nbjm(
    block: dict,
    registry: IdRegistry,
    level: int = 0,
    mode: str = "edit"
) -> list[str]:
    """Render a Notion block to NBJM format.
    ...
    Returns:
        List of NBJM lines for this block and its children.
    """
    lines = []
    block_id = block.get("id", "")
    ...
    if block_type in OPAQUE_BLOCK_TYPES:
        ...
        if "caption" in type_data:
            caption = notion_rich_text_to_nbjm(type_data["caption"])

        placeholder = f"!{block_type}~"
        if caption:
            placeholder += f" ({caption})"

        _append_nbjm_line(lines, mode, short_id, indent, placeholder)
        return lines
    ...
    if block_type == "divider":
        _append_nbjm_line(lines, mode, short_id, indent, "---")
        return lines
    ...
    if block_type == "column_list":
        ...
        _append_nbjm_line(lines, mode, short_id, indent, f"!cols {num_columns}")

        # Render each column with its number
        for i, column in enumerate(children, 1):
            lines.extend(render_column_to_nbjm(column, registry, level + 1, mode, i))
    ...
    if block_type == "table":
        ...
        _append_nbjm_line(lines, mode, short_id, indent, f"!table {table_width}")
    ...
    if block_type == "table_row":
        ...
        cell_texts = []
        for cell in cells:
            cell_nbjm = notion_rich_text_to_nbjm(cell)
            # Escape literal pipes in cell content
            cell_nbjm = cell_nbjm.replace('|', '\\|')
            cell_texts.append(cell_nbjm)
        row_content = "| " + " | ".join(cell_texts) + " |"
        _append_nbjm_line(lines, mode, short_id, indent, row_content)
        return lines
    ...
    if block_type == "code":
        ...
        _append_nbjm_line(lines, mode, short_id, indent, f"{fence}{language}")
    ...
    if block_type == "to_do":
        ...
        rich_text = type_data.get("rich_text", [])
        content = notion_rich_text_to_nbjm(rich_text)
        _append_nbjm_line(lines, mode, short_id, indent, f"{marker}{content}")

    elif block_type == "child_page":
        title = type_data.get("title", "Untitled")
        _append_nbjm_line(lines, mode, short_id, indent, f"§ {title}")
...
def render_page_to_nbjm(
    page: dict,
    blocks: list[dict],
    registry: IdRegistry,
    mode: str = "edit"
) -> str:
    """Render a complete page to NBJM format.
    ...
    # Render blocks
    for block in blocks:
        lines.extend(render_block_to_nbjm(block, registry, level=0, mode=mode))

    return "\n".join(lines)


def render_blocks_to_nbjm(
    blocks: list[dict],
    registry: IdRegistry,
    mode: str = "edit"
) -> str:
    """Render a list of blocks to NBJM format (no page header).
    ...
    Returns:
        NBJM formatted string of the blocks.
    """
    lines = []
    lines.append("@nbjm 1")
...
def render_database_to_nbjm(
    ...
    """Render a database to NBJM format with TSV rows.
    ...
    Returns:
        Complete NBJM database document string.
    """
    lines = []
    ...
    for name in prop_names:
        ...
        nbjm_type = PROPERTY_TYPE_MAP.get(prop_type, prop_type)
        ...
        if any(c in name for c in ' \t()[]"\\'):
            ...
            col_defs.append(f'"{escaped_name}"({nbjm_type})')
        else:
            col_defs.append(f"{name}({nbjm_type})")
...
class ApplyOp:
    ...
    content_blocks: list[NbjmBlock] = field(default_factory=list)
...
def _check_missing_payload(
    ...
    Returns:
        NbjmParseError if payload is missing/non-indented, None otherwise.
    """
    if i >= len(lines) or not lines[i].strip():
        # EOF or blank line — payload missing entirely
        return NbjmParseError(
            code="PARSE_ERROR",
        ...
        if match:
            ...
            while i < len(lines):
                ...
                if in_code_block:
                    ...
                    break
            # Parse content as NBJM blocks (without IDs)
            if content_lines:
...
def _parse_content_blocks(lines: list[str], errors: list[NbjmParseError], start_line: int) -> list[NbjmBlock]:
    """Parse indented content lines into NBJM blocks.
...
def _auto_group_table_rows(blocks: list[NbjmBlock]) -> list[NbjmBlock]:
...
def nbjm_block_to_notion(block: NbjmBlock, registry: IdRegistry) -> dict:
    """Convert a NbjmBlock to Notion API block format.

    Args:
        block: Parsed NBJM block.
    ...
    # These blocks only contain rich_text with no additional properties.
        ...
        for child in block.children:
            if child.block_type == 'table_row':
                row_children.append(nbjm_block_to_notion(child, registry))
            # Skip separator rows
        return {
            "type": "table",
            "table": {
...
def build_block_tree(blocks: list[NbjmBlock], registry: IdRegistry) -> list[dict]:
    """Build nested Notion block structure from flat NbjmBlock list.

    Handles indentation levels to create parent-child relationships.

    Args:
        blocks: Flat list of NbjmBlocks with level attributes.
    ...
    for block in blocks:
        notion_block = nbjm_block_to_notion(block, registry)
        level = block.level
...
async def update_block_text_async(
    ...
    Args:
        ...
        new_text: New NBJM-formatted text content (may include block markers).
    ...
    if needs_replace:
        ...
        new_notion_block = nbjm_block_to_notion(new_nbjm_block, registry)
...
def _build_property_value(
    ...
    """Convert a NBJM value to Notion property format based on property type.

    Args:
        prop_type: Notion property type (title, rich_text, select, etc.)
        value: String value from NBJM row
...
def _build_row_properties(
    ...
    Args:
        row_values: Dict of column_name -> value from NBJM
    ...
    for col_name, value in row_values.items():
        ...
        if not prop_def:
            ...
            # but NBJM input is stripped during parsing, so we normalize here.
...
async def _read_impl(
    ...
    Returns NBJM formatted content or an error string.
    """
    global _id_registry
    ...
    try:
        ...
        nbjm = render_page_to_nbjm(page, blocks, _id_registry, mode=mode)
        return nbjm
        ...
        if e.response is not None:
            if e.response.status_code in (404, 400):
                ...
                try:
                    ...
                    # Render as NBJM (block mode - no page header)
                    nbjm = render_blocks_to_nbjm(blocks, _id_registry, mode=mode)
                    return nbjm
                except Exception:
...
async def notion_read(
    ...
    """Read Notion pages or databases in compact NBJM format.

    Pages return NBJM block content (headings, lists, toggles, etc.).
    ...
    Returns:
        NBJM formatted content. Multiple pages separated by ▤ headers.
